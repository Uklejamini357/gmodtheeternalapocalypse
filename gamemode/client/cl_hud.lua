...
--XP Box
	...
	--EXperience
	draw.SimpleText( "XP: " .. math.floor( Myxp ) .. "/" .. GetReqXPClient( me ), "TargetIDSmall", 270, ScrH() - 86, Color( 205, 205, 205, 255 ), 0, 1 )
	--Skill Points?
	draw.SimpleText( translate.Get("Pts")..": " .. math.floor(Mypoints), "TargetIDSmall", 270, ScrH() - 68, Color( 205, 205, 205, 255 ), 0, 1 )
	--Bounty
	draw.SimpleText( translate.Get("Bounty")..": " .. math.floor(Mybounty), "TargetIDSmall", 270, ScrH() - 50, Color( 205, 205, 205, 255), 0, 1 )
end
	...
	if ( LocalPlayer():Alive() ) then
		...
		if LocalPlayer():IsValid() then
			if GetConVar("tea_cl_hudstyle"):GetInt() <= 0 then
				...
				if Spawn > CurTime() + 1 then
					draw.DrawText(translate.Get("CanRespawnIn") .." " .. math.Clamp( math.floor( (Spawn - CurTime()) + 1 ), 0, 2147483647 ) .. " "..translate.Get("Seconds"), "TargetID", ScrW( ) / 2 - 108, ScrH() / 2 - 200, Color( 255,255,255,255 ), 0 )
				elseif Spawn > CurTime() then
					draw.DrawText(translate.Get("CanRespawnIn") .." " .. math.Clamp( math.floor( (Spawn - CurTime()) + 1 ), 0, 2147483647 ) .. " "..translate.Get("Second"), "TargetID", ScrW( ) / 2 - 108, ScrH() / 2 - 200, Color( 255,255,255,255 ), 0 )
				else
