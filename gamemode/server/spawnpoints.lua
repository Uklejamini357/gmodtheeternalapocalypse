...
function GM:LoadPlayerSpawns()
	...
	if file.Exists(GAMEMODE.DataFolder.."/spawns/"..string.lower(game.GetMap()).."/players.txt", "DATA") then
		PlayerSpawnsData = ""
		PlayerSpawnsData = file.Read(GAMEMODE.DataFolder.."/spawns/"..string.lower(game.GetMap()).."/players.txt", "DATA")
...
hook.Add("PlayerSpawn", "ate_whoknowswhattocallhooksthesedays", GM.SpawnPlayer)

function GM.AddPlayerSpawn(ply, cmd, args, str)
	if !SuperAdminCheck(ply) then
		SystemMessage(ply, translate.ClientGet(ply, "superadmincheckfail"), Color(255,205,205), true)
		ply:ConCommand("playgamesound buttons/button8.wav")
		return
	end

	if !str or str == "" then
		SystemMessage(ply, "Specify a spawnpoint name!", Color(255,205,205), true)
		ply:ConCommand("playgamesound buttons/button8.wav")
		return
	end
	...
	tea_DebugLog("[SPAWNPOINTS MODIFIED] "..ply:Nick().." has added a new player spawnpoint called '"..str.."' at position "..tostring(ply:GetPos()).."!")
...
function GM.ClearPlayerSpawns(ply, cmd, args)
	...
	tea_DebugLog("[SPAWNPOINTS REMOVED] "..ply:Nick().." has deleted all player spawnpoints!")
	ply:ConCommand("playgamesound buttons/button15.wav")
end
concommand.Add("tea_clearplayerspawnpoints", GM.ClearPlayerSpawns)
