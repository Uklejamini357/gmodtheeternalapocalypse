...
SWEP.ViewModelBoneMods = {
	["ValveBiped.Bip01_R_Hand"] = { scale = Vector(0.068, 0.068, 0.068), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) }
}

SWEP.VElements = {
	["sword"] = { type = "Model", model = "models/props_c17/TrapPropeller_Blade.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(0.663, 0.159, -0.013), angle = Angle(12.682, -4.148, 85.522), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["sword"] = { type = "Model", model = "models/props_c17/TrapPropeller_Blade.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.094, 0.476, -2.639), angle = Angle(-31.952, -14.528, 82.214), size = Vector(0.43, 0.43, 0.43), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
/*---------------------------------------------------------
   Name: SWEP:Precache()
---------------------------------------------------------*/
...
function SWEP:PrimaryAttack()
	...
	if ((CLIENT and Mystamina < 12.5) or (SERVER and self.Owner.Stamina < 12.5)) then return end
	...
	if ( !IsValid( self ) || !IsValid( self.Owner ) || !self.Owner:GetActiveWeapon() || self.Owner:GetActiveWeapon() != self || CLIENT ) then return end
		...
		self.Owner.Stamina = math.Clamp(self.Owner.Stamina - math.Rand(10, 11.5), 0, 100)
	...
	self.Owner:ViewPunch( Angle(-1, -1, 0.5) )
	end )

	timer.Simple( 0.2, function()
	if ( !IsValid( self ) || !IsValid( self.Owner ) || !self.Owner:GetActiveWeapon() || self.Owner:GetActiveWeapon() != self ) then return end
	self.Owner:ViewPunch( Angle( math.Rand(2,6), 3, -2 ) )
	end )
...
function SWEP:DealDamage( anim )
	...
	if tr.Hit then 
		if tr.Entity:IsPlayer() or tr.Entity:IsNPC() or tr.Entity.Type == "nextbot" then 
			self.Weapon:EmitSound( "physics/body/body_medium_break2.wav", 80, math.Rand(75, 95) )
		else
			self.Weapon:EmitSound( "physics/metal/metal_solid_impact_bullet3.wav", 60, math.Rand(60, 70) )
		end
	end
