...
function ENT:Use( activator, caller )
	...
	local qty = self.LootQuantity or GAMEMODE.LootTableBoss[name]["Qty"]
	...
	caller:SendChat("You picked up a boss drop cache containing [ "..qty.."x "..GAMEMODE:GetItemName(name, caller).." ]")
	-- caller:SendChat(translate.ClientFormat(caller, "you_picked_up_a_lootcache_boss", qty, GAMEMODE:GetItemName(name, caller)))
	for _,ply in pairs(player.GetAll()) do
		ply:SystemMessage(caller:Nick().." has found a boss cache containing "..qty.."x "..GAMEMODE:GetItemName(name, ply).."!", Color(255,255,255,255), true)
		-- ply:SystemMessage(translate.ClientFormat(ply, "player_found_lootcache_boss", caller:Nick(), qty, GAMEMODE:GetItemName(name, ply)), Color(255,255,255), true)
	end
