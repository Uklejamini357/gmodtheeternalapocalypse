...
hook.Add("PostDrawTranslucentRenderables", "TEADrawDamage", function()
...
--	end
	...
	for _, particle in pairs(Particles) do
		if particle and curtime < particle.DieTime then
			local c = Color(230,110,0)

			done = false

			c.g = math.Clamp(c.g - (particle.Amount * 0.4), 0, 255)
			c.a = math.Clamp(particle.DieTime - curtime, 0, 1) * 220

			cam.Start3D2D(particle:GetPos(), ang, 0.1 * (GAMEMODE.DamageNumberScale or 1))
				draw.SimpleText(math.Round(particle.Amount, 1), "DamageFloaterFont", 0, 0, c, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
			cam.End3D2D()
		end
	end
...
function EFFECT:Init(data)
	...
	local velscal = GAMEMODE.DamageNumberSpeed or 1
	...
	particle.DieTime = CurTime() + 2 * (GAMEMODE.DamageNumberLifetime or 1)
	particle.Type = Type
