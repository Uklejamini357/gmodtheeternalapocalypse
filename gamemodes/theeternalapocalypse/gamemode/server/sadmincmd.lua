...
GM.SuperAdminCmdCmds = GM.SuperAdminCmdCmds or {}

function GM:RegisterSuperAdminCmd(name, callback)
	self.SuperAdminCmdCmds[name] = callback
end
...
function GM:AdminCmds_ClearZeds(ply, cmd, args)
	...
	if args[1] == "force" then
		...
		for k, v in pairs(ents.GetAll()) do
			if v.Type == "nextbot" or (v:IsNPC() and v:GetClass() != "tea_trader") then v.LastAttacker = nil c = c + 1 v:Remove() end
		end
