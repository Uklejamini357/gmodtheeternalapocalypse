...
GM.TestMode = CreateConVar("tea_testmode", 0, FCVAR_REPLICATED + FCVAR_ARCHIVE, "Test mode. Disables specific checks. Don't enable it on public servers, unless you want players to see development stuff.", 0, 1):GetBool()
cvars.AddChangeCallback("tea_testmode", function(cvar,old,new)
	GAMEMODE.TestMode = tobool(new)
	BroadcastLua(Format("GAMEMODE.RespawnTime = %s", tobool(new)))
	GAMEMODE:OnConVarChanged(cvar, old, new)
end, "TEA_GAMEMODE.ConVarsChangeCallbacks.tea_testmode")
