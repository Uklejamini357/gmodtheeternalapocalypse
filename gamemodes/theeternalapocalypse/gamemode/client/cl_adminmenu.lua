...
--			GiveButton.DoDoubleClick = GiveButton.DoClick
	...
	column = TransitionsView:AddColumn("Linked To")
	column:SetMaxWidth(70)
	column = TransitionsView:AddColumn("Map")
	column = TransitionsView:AddColumn("Linked Map")
	column = TransitionsView:AddColumn("Name")
	...
	frame.OnReceiveTransitionsData = function(self, tbl)
		...
		for id,v in pairs(tbl) do
			...
			line.OnRightClick = function()
				local derma = DermaMenu()
				if v.LinkedTo then
					derma:AddOption("Unlink from another transition", function()
						Derma_Query("Delete the link from ID #"..id.."?","sub","Yes", function(str)
							net.Start("tea_openworld_level")
					...
					derma:AddOption("Link to...", function()
				...
				derma:AddOption("Rename", function()
					Derma_StringRequest("blah","sub",var.Name, function(str)
				...
				derma:AddOption("Delete transition", function()
					Derma_Query("This action cannot be reversed. Are you sure?", "WARNING", "Yes", function()
						net.Start("tea_openworld_level")
