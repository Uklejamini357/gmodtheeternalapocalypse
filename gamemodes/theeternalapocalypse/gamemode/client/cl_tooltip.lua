...
function PANEL:Paint( w, h )
	...
	if self.Closing then
		ToolTipAlphaMul = math.Approach(ToolTipAlphaMul, 0, (2 - ToolTipAlphaMul) * RealFrameTime()*4)
	else
		ToolTipAlphaMul = math.Approach(ToolTipAlphaMul, 1, RealFrameTime()*3)
	end
	self:SetAlpha(255*ToolTipAlphaMul)

	self:PositionTooltip()

	draw.RoundedBox(12, 0, 0, w, h, Color(0,0,0,205))

	if self.Closing and ToolTipAlphaMul == 0 then
