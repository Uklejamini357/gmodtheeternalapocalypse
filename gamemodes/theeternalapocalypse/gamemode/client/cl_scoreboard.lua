...
-----------------------------------------Main Sheet---------------------------------------------------------------
	...
	local function SetUpScoreBoard()
		for k, v in pairs(player.GetAll()) do
			...
			local plystatus = vgui.Create("DLabel", plypanel)
			plystatus:SetPos(430, 32)
			plystatus:SetFont("TEA.HUDFontSmall")
			plystatus:SetColor(Color(255,255,255))
			plystatus:SetText(translate.Format("status", v:Alive() and translate.Get("status_alive") or translate.Get("status_dead")))
