...
---------------- HEALTH ----------------
    ...
    draw.SimpleText(translate.Format("health", hp, mhp), "TEA.HUDFontSmall", 21, h - 210, Color(205,205,205,255), 0, 1)
...
---------------- ARMOR ----------------
    ...
    draw.SimpleText(translate.Format("armor", ap, map), "TEA.HUDFontSmall", 21, h - 180, Color(205,205,205,255),0,1)
    ...
    if weapon != NULL then
        if (swep.AmmoClip1 != -1 or swep.AmmoClip2 != -1) then
            ...
            if (swep.AmmoClip2 != -1) then
                draw.SimpleText("Ammo in Clip: ".. swep.AmmoClip1 .." / ".. swep.AmmoClip2, "TEA.HUDFontSmall", 270, h - 210, Color(205,205,205,255), 0, 1) 
            else
                draw.SimpleText("Ammo in Clip: ".. swep.AmmoClip1, "TEA.HUDFontSmall", 270, h - 210, Color(205,205,205,255), 0, 1) 
            end
            --Second Clip Ammo Text
            draw.SimpleText("Ammo Remaining: ".. swep.MaxAmmoType, "TEA.HUDFontSmall", 270, h - 192, Color(205,205,205,255), 0, 1)
            --Alt ammo Text
            draw.SimpleText("ALT: ".. swep.MaxAmmoType2, "TEA.HUDFontSmall", 270, h - 174, Color(205,205,205,255), 0, 1)
        else IsAmmoBox = false end
    end
-------------- EXperience --------------
    surface_SetDrawColor(0, 0, 0, 255)
    surface_DrawOutlinedRect(w - 250, h - 50, 200, 8)
    draw.SimpleText(Format("XP: %s/%s (%s%%)", math.floor(MyXP), pl:GetReqXP(), math.Round(math.floor(MyXP) * 100 / pl:GetReqXP())), "TEA.HUDFontSmall", w - 250, h - 60, Color(205, 205, 205, 255), 0, 1)
...
-------------- Stamina -------------- 
    ...
    draw.SimpleText(translate.Format("stamina", math.Round(pl.Stamina, GAMEMODE.HUDDecimalValues and 1 or 0)), "TEA.HUDFontSmall", 20, h - 150, Color(205,205,205,255), 0, 1)
...
-------------- Thirst --------------
    ...
    draw.SimpleText(translate.Format("thirst", math.Round(pl.Thirst / 100, GAMEMODE.HUDDecimalValues and 1 or 0)), "TEA.HUDFontSmall", 20, h - 120, Color(205,205,205,255), 0, 1)
...
-------------- Hunger --------------
    ...
    draw.SimpleText(translate.Format("hunger", math.Round(pl.Hunger / 100, GAMEMODE.HUDDecimalValues and 1 or 0)), "TEA.HUDFontSmall", 20, h - 90, Color(205,205,205,255), 0, 1)
...
-------------- Fatigue --------------
    ...
    draw.SimpleText(translate.Format("fatigue", math.Round(pl.Fatigue / 100, GAMEMODE.HUDDecimalValues and 1 or 0)), "TEA.HUDFontSmall", 20, h - 60, Color(205,205,205,255), 0, 1)
...
-------------- Infection --------------
    ...
    draw.SimpleText(translate.Format("infection", math.Round(pl.Infection / 100, GAMEMODE.HUDDecimalValues and 1 or 0)), "TEA.HUDFontSmall", 20, h - 30, Color(205,205,205,255), 0, 1)
...
-------------- Battery --------------
    surface_SetDrawColor(0, 0, 0, 255)
    surface_DrawOutlinedRect(w - 250, h - 20, 200, 8)
    local armorstr = pl:GetNWString("ArmorType") or "none"
    local armortype = GAMEMODE.ItemsList[armorstr]
    draw.SimpleText(translate.Format("battery", math.max(pl.Battery, 0), pl:GetMaxBattery()), "TEA.HUDFontSmall", w - 250, h - 30, Color(205,205,205,255), 0, 1)
    surface_DrawRectColor(w - 250, h - 20, 200, 8, Color(0,0,75,75))
    if pl.Battery > 0 then
        local batterybarclamp = math.Clamp((pl.Battery * 2) / (pl:GetMaxBattery() / 100), 0, 200)
        surface_DrawRectColor(w - 250, h - 20, batterybarclamp, 4, Color(0,0,200,160))
        surface_DrawRectColor(w - 250, h - 16, batterybarclamp, 4, Color(0,0,150,160))
    end
----------------Levels, cash, prestige & time survived----------------
    draw.SimpleText(translate.Format("prestige", math.floor(pl:GetTEAPrestige())), "TEA.HUDFontSmall", 270, h - 140, Color(205,205,205,255), 0, 1)
    draw.SimpleText(translate.Format("level", math.floor(pl:GetTEALevel())), "TEA.HUDFontSmall", 270, h - 122, Color(205,205,205,255), 0, 1)
    draw.SimpleText(translate.Format("money", math.floor(MyMoney)), "TEA.HUDFontSmall", 270, h - 104, Color(205,205,205,255), 0, 1)
    draw.SimpleText(translate.Format("bounty", math.floor(MyBounty)), "TEA.HUDFontSmall", 270, h - 86, Color(205, 205, 205, 255), 0, 1)
end

function HUD:DrawPVP(pl, state, w, h)
    draw.SimpleText("PvP: "..translate.Get("pvp_state"..state), "TEA.HUDFontSmall", 270, h - 30, Color(205,205,205), 0, 0)
end
