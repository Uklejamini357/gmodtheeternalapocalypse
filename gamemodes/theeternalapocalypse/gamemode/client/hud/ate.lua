...
---------------- HEALTH ----------------
    ...
    surface.DrawOutlinedRect(10, h - 110, 180, 100)
    draw.SimpleText(translate.Format("health", hp, mhp), "TEA.HUDFontSmall", 21, h - 96, Color(math.Clamp(255 * (hp / mhp), 127, 255),48,48,255), 0, 1)
...
---------------- ARMOR ----------------
    draw.SimpleText(translate.Format("armor", ap, map), "TEA.HUDFontSmall", 21, h - 50, Color(48,48,math.Clamp(255 * (ap / map), 127, 255),255), 0, 1)
    ...
    surface.DrawOutlinedRect(20, h - 36,160,20)
    surface.DrawOutlinedRect(19, h - 37,162,22)
...
---------------- AMMO ----------------
    ...
    if weapon != NULL then
        if (swep.MaxAmmoType != 0 or swep.MaxAmmoType2 != 0 or swep.AmmoClip1 > 0 or swep.AmmoClip2 > 0) then
            ...
            surface.DrawOutlinedRect(w - 270, h - 140, 250, 70)

            --Ammo Text
            if (swep.AmmoClip2 != -1) then
                draw.SimpleText("Ammo in Clip: ".. swep.AmmoClip1 .." / ".. swep.AmmoClip2, "TEA.HUDFontSmall", w - 259, h - 110, Color(255, 255, 255, 255), 0, 1)
            else
                draw.SimpleText("Ammo in Clip: ".. swep.AmmoClip1, "TEA.HUDFontSmall", w - 259, h - 110, Color(255, 255, 255, 255), 0, 1) 
            end
            ...
            draw.SimpleText("Ammo Remaining: ".. swep.MaxAmmoType, "TEA.HUDFontSmall", w - 259, h - 130, Color(255, 255, 255, 255), 0, 1)

            --Alt ammo Text
            draw.SimpleText("ALT: ".. swep.MaxAmmoType2, "TEA.HUDFontSmall", w - 89, h - 90, Color(255, 255, 255, 255), 0, 1)
...
-------------- EXperience --------------
    ...
    surface.DrawOutlinedRect(w - 270, h - 60, 250, 50)

    draw.SimpleText(Format("XP: %s/%s (%s%%)", math.floor(MyXP), pl:GetReqXP(), math.Round(math.floor(MyXP) * 100 / pl:GetReqXP())), "TEA.HUDFontSmall", w - 259, h - 48, Color(255, 255, 255, 255), 0, 1)
    ...
    surface.DrawOutlinedRect(200, h - 200, 180, 190)

-------------- Stamina --------------
    draw.SimpleText(translate.Format("stamina", math.Round(pl.Stamina, GAMEMODE.HUDDecimalValues and 1 or 0)), "TargetIDTiny", 210, h - 186, !cansprint and Color(255, 155, 105) or Color(205, 255, 205), 0, 1)
...
-------------- Hunger --------------
    draw.SimpleText(translate.Format("hunger", math.Round(pl.Hunger / 100, GAMEMODE.HUDDecimalValues and 1 or 0)), "TargetIDTiny", 210, h - 150, Color(255, 205, 255, 255), 0, 1)
...
-------------- Thirst --------------
    draw.SimpleText(translate.Format("thirst", math.Round(pl.Thirst / 100, GAMEMODE.HUDDecimalValues and 1 or 0)), "TargetIDTiny", 210, h - 114, Color(155,155,255,255), 0, 1)
...
-------------- Fatigue --------------
    draw.SimpleText(translate.Format("fatigue", math.Round(pl.Fatigue / 100, GAMEMODE.HUDDecimalValues and 1 or 0)), "TargetIDTiny", 210, h - 78, Color(205,205,255,255), 0, 1)
...
-------------- Infection --------------
    draw.SimpleText(translate.Format("infection", math.Round(pl.Infection / 100, GAMEMODE.HUDDecimalValues and 1 or 0)), "TargetIDTiny", 210, h - 42, Color(205, 105, 105, 255), 0, 1)
...
-------------- Battery --------------
    draw_RoundedBox(0, 140, 30, 180, 45, Color(0, 0, 0, 175))
    surface_SetDrawColor(0, 0, 60, 205)
    surface.DrawOutlinedRect(140, 30, 180, 45)
    local armorstr = pl:GetNWString("ArmorType") or "none"
    local armortype = GAMEMODE.ItemsList[armorstr]
    draw.SimpleText(translate.Format("battery", math.Clamp(pl.Battery, 0, math.huge), pl:GetMaxBattery()), "TargetIDTiny", 150, 42, Color(5,5,255,255), 0, 1)
    draw_RoundedBox(2, 150, 52, 160, 15, Color(0, 0, 80, 100))
    if pl.Battery > 0 then
        local batterybarclamp = math.Clamp((pl.Battery * 1.6) / (pl:GetMaxBattery() / 100), 0, 160)
        draw_RoundedBox(4, 150, 52, batterybarclamp, 15, Color(0, 0, 250, 160))
        draw_RoundedBox(4, 150, 52, batterybarclamp, 8, Color(0, 0, 50, 100))
    end
...
----------------Levels, cash & prestige----------------
    ...
    surface.DrawOutlinedRect(10, h - 200, 180, 85)
    draw.SimpleText(translate.Format("bounty", math.floor(MyBounty)), "TEA.HUDFontSmall", 20, h - 187, Color(155, math.Clamp(127 - (MyBounty / 60), 0, 255), math.Clamp(255 - (MyBounty / 30), 0, 255)), 0, 1)
    draw.SimpleText(translate.Format("prestige", math.floor(pl:GetTEAPrestige())), "TEA.HUDFontSmall", 20, h - 168, Color(205,155,255), 0, 1)
    draw.SimpleText(translate.Format("level", math.floor(pl:GetTEALevel())), "TEA.HUDFontSmall", 20, h - 149, Color(200,200,200), 0, 1)
    draw.SimpleText(translate.Format("money", math.floor(MyMoney)), "TEA.HUDFontSmall", 20, h - 130, Color(0,255,255), 0, 1)
end
...
function HUD:DrawPVP(pl, state, w, h)
    ...
    draw.SimpleText("PvP: "..translate.Get("pvp_state"..state), "TEA.HUDFontSmall", 180, 86, Color(205,205,205), 0, 0)
    if state == 2 or state == 3 or state == 4 then
        draw.SimpleTextOutlined("C", "CSSTextFont", 135, 85, Color(255, 50, 0), 0, 0, 2, Color(50, 0, 0))
    elseif state == 1 then
        draw.SimpleTextOutlined("p", "CSSTextFont", 145, 83, Color(50, 250, 0), 0, 0, 2, Color(0, 50, 0))
    else
        draw.SimpleTextOutlined("C", "CSSTextFont", 135, 85, Color(50, 50, 50), 0, 0, 2, Color(20, 0, 0))
    end
end
